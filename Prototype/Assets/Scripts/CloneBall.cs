...
public class CloneBall : MonoBehaviour
{
    ...
    private bool isBomb;
    private LowestRowTrigger lowestRowController;
    ...
    {
        returnVelocity = 1;
        lowestRowController = GameObject.Find("LevelContainer").GetComponent<LowestRowTrigger>();
        rowInfo = GameObject.Find("LevelContainer").GetComponent<RowGeneration>();
    ...
    private void FixedUpdate()
    {
        if (!gameOver)
        {
            ...
            {
                rigid.velocity = controller.cloneSpeed * (rigid.velocity.normalized) * controller.speedMultiplier * returnVelocity;
            }
    ...
    bool CheckPassed()
    {
        ...
        if (lowestRowController.LowestPosition)
        {
            float newLower = lowestRowController.LowestPosition.position.y - rowInfo.DISTANCE_BETWEEN_BLOCKS;
            if (rigid.velocity.y < 0 && transform.position.y < newLower)
            {
                returnVelocity = 2;
                return true;
            }
